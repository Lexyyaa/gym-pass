...
|---|---|
| F0 설계 | `00-assignment` → `01-analysis` → 결정 → `02` → `03` → `04` |
| F1 기초 설정 | `03` §1 용어·§6 ERD·§9 seed / `04` §4 에러 코드 |
| 기능 구현 | `02` 해당 기능 절 / `03` 해당 애그리거트·§4 상태 전이·§7 동시성 / `04` 해당 API |
| 테스트 작성 | `02` 해당 기능의 TC 표 / `04` 해당 API의 Errors |
| F9 마무리 | `02` §4 제출물·§5 결정 / `04` 전체 / `task_list` 보류 목록 |
...
## 상시 결정 — `C-n` 없이 바로 적용한다

아래에 해당하는 빈틈은 사용자에게 묻지 않는다.

- 적용한 사람(구현자 · 문서 작성자)은 보고의 "설계에 없어서 정한 것"에 적는다.
- 리뷰 · 검증은 이 규칙을 따른 판단을 "결정 누락"으로 지적하지 않는다.

**입력 범위**
- 모든 입력은 DB 타입 범위 안에서만 받는다.
  - DATE: 1000-01-01 ~ 9999-12-31
  - 문자열: 컬럼 길이
  - 정수: 컬럼 타입 범위
- 범위를 벗어나면 도메인에서 `{도메인}_INVALID_INPUT`을 던진다.

**정책 수치**
- 업무 한도 수치(최대 개수 · 기간 · 횟수)는 `application.yml` + `@ConfigurationProperties`에 둔다.
- 원문에 없는 수치는 README "가정"에 근거와 함께 적는다.

**요청 필드**
- 요청 종류에 해당하지 않는 필드가 값을 갖고 오면 거부한다. 무시하지 않는다.

**엔티티**
- 애그리거트 간 참조는 식별자(Long)만 두고, 물리 FK를 걸지 않는다.
  - 같은 애그리거트 안의 연관만 JPA 연관으로 둔다.
- 모든 엔티티는 `BaseTimeEntity`를 상속한다.
  - ERD의 시각 컬럼 표기와 달라도 불일치로 보지 않는다.

**결정 기록 위치**
- 결정의 원문(결정 · 근거 · 한계)은 `02` §5 `D-n`에만 적는다.
- `03` · `04`에는 계약 값(수치 · 에러 코드 · 조건)과 `(D-n)` 참조만 적는다.
  - 이유 · 선택지 · 한계를 다시 서술하지 않는다.

## 하지 말 것

- `docs/design/00-assignment.md` 수정 (과제 원문)
- 사용자 결정 없이 분석의 `C-n`을 정하기
  - 결정의 원본 기록은 `docs/design/01-analysis.md` §7이다
  - `02` §5의 `D-n`은 거기서 옮긴 것이다
- 스펙에 없는 기능·필드·API 추가
...
- ArchUnit
  ...
  - `src/test/java/com/gym/pass/architecture/ArchitectureTest.java`
...
- 부분 성공 API를 상태코드로만 검증한다
  - → 200인데 저장 0건인 거짓 통과
- 날짜 입력의 상한만 막고 하한을 빠뜨린다
  - → MySQL DATE는 1000-01-01 ~ 9999-12-31. 음수 연도는 에러 없이 다른 날짜로 저장된다
  - 양쪽을 짝으로 막고, 경계값은 저장 후 다시 읽어 단언한다
- 락 → 검사 → 삽입에서 락 앞에 일반 SELECT를 둔다
  - → REPEATABLE READ 스냅샷이 락 이전에 잡혀, 먼저 커밋된 행을 못 보고 중복이 생긴다
  - 락 조회를 트랜잭션의 첫 쿼리로 둔다
- "무시한다"고 정한 입력 필드에 Bean Validation을 남긴다
  - → 무시될 값 때문에 400이 난다
- `insertable = false`로 이중 매핑한 FK 필드를 저장 직후 읽는다
  - → 메모리 값이 null이다. 연관에서 꺼내거나 다시 조회한다
- 락을 두 단계로 잡을 때 뒤의 락이 스냅샷을 새로 잡아 준다고 착각한다
  - → REPEATABLE READ 스냅샷은 첫 일반 조회 시점에 고정된다
  - 정합성의 근거는 그 조회보다 앞에 잡은 락이다
- `FOR UPDATE` 전에 같은 엔티티를 일반 조회로 영속성 컨텍스트에 올린다
  - → 잠금 조회가 새로 읽은 값 대신 1차 캐시의 옛 값을 돌려준다
  - 후보는 id만 조회하고, 엔티티는 잠금 조회로 처음 적재한다
- 범위 조건으로 `FOR UPDATE`를 건다
  - → 대상 행이 없어도 인접 인덱스 구간에 갭 락이 걸려 다른 키의 INSERT가 대기한다
  - 락은 PK 등호 조회로 잡는다
- 날짜 계산(`plusDays` · `plusMonths`)을 범위 검사보다 먼저 한다
  - → ISO 파싱은 `+999999999-12-31`까지 받아 계산이 `DateTimeException`(500)을 낸다
  - 입력 날짜의 상한 · 하한을 계산보다 먼저 막는다
- 하위 기간(정지 등)이 부모 기간 안에 있는지 검사하지 않는다
  - → 부모 기간 밖의 하위 기간이 부모 종료일만 늘려 규칙에 없는 혜택이 생긴다
  - 등록뿐 아니라 해제처럼 부모 기간이 줄어드는 경로에서도 다시 검사한다
- 여러 날 구간의 포함 여부를 시작일로만 비교하고, 경계 테스트를 1일짜리 구간으로만 만든다
  - → 구간 끝이 기준일을 넘는 경우를 놓친다
  - 끝 날짜로 비교하고, 경계 테스트에 2일 이상 구간을 넣는다
- 새 스냅샷이 필요한데 `TransactionTemplate`를 기본 전파(REQUIRED)로 쓴다
  - → 바깥 트랜잭션에 합류해 락보다 먼저 잡힌 스냅샷을 그대로 쓴다
  - `REQUIRES_NEW`로 두고, "호출자에 트랜잭션이 없어야 한다"는 전제를 클래스 문서에 적는다
